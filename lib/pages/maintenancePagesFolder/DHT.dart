...
class DHTMaintenancePage extends StatefulWidget {
  @override
  _DHTMaintenancePageState createState() => _DHTMaintenancePageState();
}

class _DHTMaintenancePageState extends State<DHTMaintenancePage> {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'DHT Sensor Accuracy Test Checklist',
              style: TextStyle(
                fontSize: 30,
                fontWeight: FontWeight.bold,
                color: Colors.white,
                  ...
                  steps: [
                    Step(
                      ...
                      content: Column(
                        ...
                        children: [
                          Text(
                            'Ensure that the sensor is properly connected to the microcontroller. Check for loose wires or poor connections.',
                            style: TextStyle(
                                fontSize: 18,
                                color: Colors.white,
                                fontFamily: 'Roboto'),
                    ...
                    Step(
                      ...
                      content: Column(
                        ...
                        children: [
                          Text(
                            'Gently wipe the sensor with a dry cloth to remove any dust or debris that may affect its performance.',
                            style: TextStyle(
                                fontSize: 18,
                                color: Colors.white,
                                fontFamily: 'Roboto'),
                    ...
                    Step(
                      ...
                      content: Column(
                        ...
                        children: [
                          Text(
                            'Verify that the sensor is placed in an environment with temperatures within its operating range.',
                            style: TextStyle(
                                fontSize: 18,
                                color: Colors.white,
                                fontFamily: 'Roboto'),
                    ...
                    Step(
                      ...
                      content: Column(
                        ...
                        children: [
                          Text(
                            'For better accuracy testing, measure the temperature and humidity from your sensor and compare it with a reliable reference (like a weather station or known environment conditions).',
                            style: TextStyle(
                                fontSize: 18,
                                color: Colors.white,
                                fontFamily: 'Roboto'),
        ...
        return WillPopScope(
          ...
          child: Dialog(
            ...
            child: VideoPlayerDialog(
                controller: _controller, onUpdate: _updateStep),
          ),
        );
      },
    );
  ...
  // Method to update step result after the video finishes
  void _updateStep(int index, bool result) {
    Navigator.of(context).pop(); // Close the video dialog
    updateStepResult(index, result); // Update the result
  }

  Widget _buildActionButton(int index, bool result, String label) {
    return ElevatedButton(
      onPressed: () {
        _updateStep(index, result); // Use the callback to update the step
      },
      child: Text(label),
      style: ElevatedButton.styleFrom(
...
class _VideoPlayerDialogState extends State<VideoPlayerDialog> {
  ...
  Widget _buildActionButton(bool result, String label) {
    return ElevatedButton(
      onPressed: () {
        widget.onUpdate(3, result); // Update the result for step 3
      },
      child: Text(label),
      style: ElevatedButton.styleFrom(
        backgroundColor: result ? Colors.green : Colors.red,
